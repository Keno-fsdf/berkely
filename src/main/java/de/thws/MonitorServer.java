...
public class MonitorServer implements Runnable {
    ...
    private int soTimeoutMs = 500; // to allow graceful stop
        ...
        try (DatagramSocket sock = new DatagramSocket(port)) {
            ...
            while (running) {
                ...
                try {
                    ...
                    continue; // periodic check of running-flag
                } catch (Exception e) {
                    if (!running) break;
                    continue;
                }
                ...
                try {
                    switch (type) {
                        ...
                        case "FailStopEnd" -> mon.onFailStopEnd(
                                Integer.parseInt(kv.get("id"))
                        );

                        case "MasterElected" -> mon.onMasterElected(
                                Integer.parseInt(kv.get("id"))
                        );

                        case "Demotion" -> mon.onDemotedTo(
                                Integer.parseInt(kv.get("newMaster"))
                        );

                        case "ElectionStart" -> mon.onElectionStart(
                                Integer.parseInt(kv.get("id"))
                        );

                        case "CoordAnnounce" -> mon.onCoordinatorAnnounce(
                                Integer.parseInt(kv.get("id"))
                        );

                        case "SyncStart" -> mon.onSyncStart(
                                Integer.parseInt(kv.get("m"))
                        );
                        ...
                        case "Drift" -> mon.updateNodeDrift(
                                Integer.parseInt(kv.get("id")),
                                Double.parseDouble(kv.get("d"))
                        );

                        // Centralize message counters (Δ/total) as well:
                        case "MsgSend" -> {
                            Message.Type tt = Message.Type.valueOf(kv.get("t"));
                            int s = Integer.parseInt(kv.get("s"));
                            int d = Integer.parseInt(kv.get("d"));
                            Message m = new Message(s, d, tt, 0, 0.0, 0.0);
                            mon.onMessageAttempt(m);
                        }
                    ...
                    // lenient server: skip malformed events
                }
            }
        } catch (Exception ignored) {
            // socket bind/close errors ignored for brevity
        }
    }
}